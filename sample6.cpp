// sample1.cpp
// esitmate the parameters of a prismatic joint
...
int main(int argc, char **argv)
{
	...
	for (size_t i(0); i < gt_motion.size(); ++i) {
		MotionMatrixd trans(Eigen::Vector3d::UnitZ() * i / (double) NUM_OF_DATA, Eigen::Matrix3d::Identity());
		gt_motion[i] = trA * trans * trB;
	}
	// add noise
	// init rand
	...
	PrismaticPair p_pair;
	EstError error;
	p_pair.Estimation(cor_trjs, trjs, error);

	std::cout << "Direction: " << p_pair.TranslationDirection(KinematicPair::BASE).transpose() << " (0 0 1) in base coordinates" << std::endl;
	std::cout << "Direction: " << p_pair.TranslationDirection(KinematicPair::TARGET).transpose() << " (0 1 0) in target coordinates" << std::endl;
	// evaluation
