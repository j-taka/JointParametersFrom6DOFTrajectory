// sample3.cpp
// esitmate the parameters of a spherical joint
...
int main(int argc, char **argv)
{
	...
	for (size_t i(0); i < gt_motion.size(); ++i){
		double theta = i * M_PI / (4.0 * NUM_OF_DATA);
		Eigen::Vector3d axis(0, sin(theta), cos(theta)); 
		MotionMatrixd rot(Eigen::Vector3d::Zero(), Eigen::AngleAxisd(i * M_PI / NUM_OF_DATA, axis).matrix());
	...
	std::cout << "Center: " << r_pair.CenterOfRotation(KinematicPair::BASE).transpose() << " (2 3 4) in base coordinates" << std::endl;
	std::cout << "Center: " << r_pair.CenterOfRotation(KinematicPair::TARGET).transpose() << " (-5 -3 4) in target coordinates" << std::endl;
	// evaluation
